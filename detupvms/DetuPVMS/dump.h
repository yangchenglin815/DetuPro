...
LONG ApplicationCrashHandler(EXCEPTION_POINTERS *pException)
{
	...
	// 构建dump文件路径
	GetLocalTime(&stTime);
    QString path = QDir::toNativeSeparators(QDir::homePath() + "/AppData/Local/Pano-VMS/Dump");
    path.toWCharArray(szDumpDir);
	//::GetCurrentDirectory(MAX_PATH, szDumpDir);
	TSprintf(szDumpFile, _T(L"%s\\%04d%02d%02d_%02d%02d%02d.dmp"), szDumpDir,
	...
	// 这里弹出一个错误对话框并退出程序
    TSprintf(szMsg, _T(L"Sorry, Pano-VMS has crashed, please contact to us for help! \r\ndump file : %s"), szDumpFile);
