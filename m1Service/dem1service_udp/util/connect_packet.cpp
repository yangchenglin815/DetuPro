...
std::string making_send_back_conv_packet(uint32_t conv)
{
    ...
    size_t n = _snprintf(str_send_back_conv, sizeof(str_send_back_conv), "%s %u", ASIO_KCP_SEND_BACK_CONV_PACKET, conv);
...
std::string making_disconnect_packet(uint32_t conv)
{
    ...
    size_t n = _snprintf(str_disconnect_packet, sizeof(str_disconnect_packet), "%s %u", ASIO_KCP_DISCONNECT_PACKET, conv);
