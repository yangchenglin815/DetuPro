...
typedef enum ModelDeviceId {
    ...
    MODEL_DEVICE_SPHERE_DETU_4 = 4001,
} ModelDeviceId;
...
typedef struct ImageData {
    ...
    CalibrationParam params[MODEL_MAX_LENS_COUNT]; //标定参数
	char calibration[MODEL_MAX_CALIBRATION_SIZE];
	char ptspath[MODEL_MAX_PTSPSTH_SIZE];
...
typedef struct PanoViewEuler
{
	...
	float x_offset;
	float y_offset;
	bool isgestureaction;
} PanoViewEuler;

typedef struct PanoRenderCallBack
{
	void(*onPanoRenderRequestChangeEuler)(void* opaque, PanoViewEuler* viewEuler,bool isflat);
} PanoRenderCallBack;
