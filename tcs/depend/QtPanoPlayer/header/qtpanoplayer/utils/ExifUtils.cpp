...
#define EXIF_ID_STR_LEN 5
    ...
    string ExifUtils::readExif(string imgAbsolutePath) {
        int ret = 0;
        string result = "";
        void **ifdArray = createIfdTableArray(imgAbsolutePath.c_str(), &ret);
        if (!ifdArray) {
            return result;
        }
        //标定信息有的在model，有的在lensModel,规则较乱
        TagNodeInfo *tag = getTagInfo(ifdArray, IFD_0TH, TAG_Model);
        if (tag) {
            //标定信息在lensmodel，model存储类似"tb401"的内容，这里兼容乐视相机，27个参数 * 2
            if (tag->count <= 27 * 2) {
                freeTagInfo(tag);
                tag = getTagInfo(ifdArray, IFD_EXIF, TAG_LensModel);
                if (tag && !tag->error) {
					result = (char*)tag->byteData;
                    freeTagInfo(tag);
                } else {
                    result = "";
                }
                ...
                if (!tag->error) {
                    result = (const char *) tag->byteData;
                } else {
                    result = "";
                }
                freeTagInfo(tag);
