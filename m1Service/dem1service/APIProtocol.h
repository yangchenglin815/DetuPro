...
#define MAX_NVR_LIST_NUM (50)			//有坑，放到50，客户端读取数据会error，待解决
...
namespace dem1 {
    namespace server {
        ...
        typedef enum
        {
			...
			RTMP_MAP_MAIN1,              //地图预览主码流
			RTMP_MAP_SUB1,                  //地图预览子码流

			RTMP_MAP_BACK_MAIN1,      //地图回放主码流
			RTMP_MAP_BACK_SUB1,         //地图回放子码流
...
#pragma pack (1) // 指定按1字节对齐
		...
		{
			unsigned  char	szDeviceId[API_ARRAY_LEN];			// 设备ID，唯一标识
			unsigned char szName[API_ARRAY_LEN];         //设备别名
			unsigned char m1DeviceId[API_ARRAY_LEN];         //设备别名
			unsigned int              szUpDown;
			unsigned int				curr_num;				// 从1开始
...
#pragma pack ()  // 取消指定对齐，恢复缺省对齐
		...
		// 心跳状态 ：CMD_HEART_BEAT
		typedef struct api_heartbeat_body
		{
			unsigned int	nInputBandWidth;				// 输入带宽：M1 --> PVS，单位：Byte/s，3s均值
			unsigned int	nOuntputBandWidth;				// 输出带宽：PVS --> 流媒体服务器
			unsigned char	nStream1Status;					// 主码流状态：0表示四路不全正常，1表示不同步，2表示正常
			unsigned char	nStream2Status;					// 子码流状态，同上
			unsigned char	nCalibrationStatus;				// 标定信息状态：0表示成功，其他表示失败
			unsigned char	nReserver[9];					// 预留
			...
			api_heartbeat_body(){
				...
				memset(&nReserver, 0, 9);
			}
			int get_size(){
				return sizeof(api_heartbeat_body);
			}
			void read(netbuffer_t* rb){
				read_uint(this->nInputBandWidth, rb);
				read_uint(this->nOuntputBandWidth, rb);
				read_uchar(this->nStream1Status, rb);
				read_uchar(this->nStream2Status, rb);
				read_uchar(this->nCalibrationStatus, rb);
				readdata(this->nReserver, 9, rb);
			}

			void write(netbuffer_t* wb){
				write_uint(this->nInputBandWidth, wb);
				write_uint(this->nOuntputBandWidth, wb);
				write_uchar(this->nStream1Status, wb);
				write_uchar(this->nStream2Status, wb);
				write_uchar(this->nCalibrationStatus, wb);
				writedata(this->nReserver, 9, wb);
			}
		}api_heartbeat_body_t;
		...
		// 时间 ：CMD_SET_M1_TIME
		typedef struct api_device_time
		{
			...
			int get_size() {
				return sizeof(api_device_time);
			}
			void read(netbuffer_t* rb) {
				read_uint(this->nYear, rb);
				read_uint(this->nMonth, rb);
				read_uint(this->nDay, rb);
				read_uint(this->nHour, rb);
				read_uint(this->nMinute, rb);
				read_uint(this->nSecond, rb);
			}

			void write(netbuffer_t* wb) {
				write_uint(this->nYear, wb);
				write_uint(this->nMonth, wb);
				write_uint(this->nDay, wb);
				write_uint(this->nHour, wb);
				write_uint(this->nMinute, wb);
				write_uint(this->nSecond, wb);
			}

		}api_device_time_t;

		// 录像策略：CMD_SET_RECORD_POLICY
		typedef struct segmentTime
		{	// 时间区间
			unsigned int nStartTime;	// 单位：分钟。如周一8:21，则取值501（8*60+21），周三23:11，则4271（2*24*60+23*60+11）
			unsigned int nEndTime;	// 同上，取值范围[0, 10080] （10080 = 7*24*60）。
			segmentTime()
			{
				nStartTime = 0;
				nEndTime = 0;
...
#define  MAX_TIME_VALUE			(10080)
#define  MAX_SEGMENT_NUM		(168)

		typedef struct api_record_policy
		{
			unsigned char			nAllDayRecord;		// 1：表示7*24h录像， 2：用户自定义，其他：无效
			unsigned char			nSegmentNum;		// 时间段总个数，最大值为168。要与segmentTime_t有效区间对应，为0表示不录像。
			unsigned char			nReserver[2];		// 预留，用于扩展
			/*
			1）下一个时间区间开始一定要大于上一个时间区间结束，否则后面的时间区间无效。
			2）时间区间[0, 0]表示不使用
			*/
			segmentTime_t	szSegmentTime[MAX_SEGMENT_NUM];	// 时间区间，每天平均24个时间段，一周最多168个。

			api_record_policy()
			{
				nAllDayRecord = 0;
				nSegmentNum = 0;
				memset(&nReserver, 0, 2);
			}
			int get_size() {
				return sizeof(api_record_policy);
			}

			void read(netbuffer_t* rb) 
			{
				read_uchar(this->nAllDayRecord, rb);
				read_uchar(this->nSegmentNum, rb);
				readdata(this->nReserver, 2, rb);
				for (int i = 0; i < this->nSegmentNum; i++)
				{
					szSegmentTime[i].read(rb);
				}
			}
			void write(netbuffer_t* wb) 
			{
				write_uchar(this->nAllDayRecord, wb);
				write_uchar(this->nSegmentNum, wb);
				writedata(this->nReserver, 2, wb);
				for (int i = 0; i < this->nSegmentNum; i++)
				{
					szSegmentTime[i].write(wb);
				}
			}
