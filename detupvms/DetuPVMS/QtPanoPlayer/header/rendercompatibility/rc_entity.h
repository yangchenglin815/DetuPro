...
#define RC_TEXTURE_TRANSFORM_MATRIX_SIZE 16 //android surfacetexture硬解数据转换矩阵
...
typedef struct RcFrame {
    ...
    //额外原始纹理转换矩阵
    GLfloat extraTextureTransformMatrix[RC_TEXTURE_TRANSFORM_MATRIX_SIZE];
} RcFrame;
...
typedef struct RcTexturesConfig {
    ...
    //android opaque会存储surfacetexture集合
    void* opaque;
} RcTexturesConfig;
