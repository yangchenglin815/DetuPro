    ...
    : QLineEdit(parent)
{
    ...
    for (int i = 0; i < 4; i++)
    {
		...
		m_lineEidt[i]->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
        pHBox->addWidget(m_lineEidt[i]);
    }
	m_lineEidt[0]->setStyleSheet("border:1px solid #c3caca;border-right:none;");
	m_lineEidt[1]->setStyleSheet("border:1px solid #c3caca;border-right:none;border-left:none;");
	m_lineEidt[2]->setStyleSheet("border:1px solid #c3caca;border-right:none;border-left:none;");
	m_lineEidt[3]->setStyleSheet("border:1px solid #c3caca;border-left:none;");
...
void QIPLineEdit::paintEvent(QPaintEvent *event)
{
    ...
    for (int i = 0; i < 3; i++)
    {
       width += m_lineEidt[i]->width() + (i == 0 ? 3 : 10);//布局的间隔
        painter.drawEllipse(width, height() / 3 * 2 , 1, 1);
    }
}
...
void QIPLineEdit::setText(const QString &strIP)
{
    ...
    else
    {
        ...
        {
            m_lineEidt[i]->setText(ipList.at(j));
            i++;
        }
    }
}
...
QString QIPLineEdit::text() const
{
    ...
    strIP.remove(strIP.size()-1, 1);
    return strIP;
}

bool QIPLineEdit::getText()
{
	QString ip1, ip2, ip3, ip4;
	ip1 = m_lineEidt[0]->text();
	ip2 = m_lineEidt[1]->text();
	ip3 = m_lineEidt[2]->text();
	ip4 = m_lineEidt[3]->text();
